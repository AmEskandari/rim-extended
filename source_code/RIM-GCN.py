...
def get_activated_node_dense(node,reliable_score,activated_node):
    ...
    argumen. It outputs number of activated node as "Count" and list of 1,0 
...
def get_max_reliable_info_node_dense(idx_used,high_score_nodes,activated_node,train_class,labels):  
    """
    Description:
    This function work for selecting the best node in each iteration
    that maximize node influence quantity and quality. in every 
    implementation of this fucntion, it selects the best and it's
    reliability and number of activated nodes by selecting this
    node.
...
def update_reliability(idx_used,train_class,labels,num_node):
    ...
    for node in idx_used:
        ...
        if node_label in train_class:   #train_class is a dict that save index of nodes that has labels in 
            ...
            for tmp_node in train_class[node_label]:
                reliable_score+=reliability_list[tmp_node]*get_reliable_score(similarity_feature[node][tmp_node]) #Numinator of Eq8 
...
def train(epoch, model,record):
    ...
    weight_one_hot_labels = torch.mul(one_hot_labels,reliability_list)#Used Reliability for Loss Function
    loss_train = my_cross_entropy(output_[idx_train],weight_one_hot_labels[idx_train]) #Put one_hot_labels instead of
    acc_train = accuracy(output[idx_train], labels[idx_train])                          # weight_one_hot_labels for training without label relaibility
    loss_train.backward()
    optimizer.step()
    model.eval()
    output = model(features_GCN, adj)
...
adj_matrix2 = torch.mm(adj_matrix,adj_matrix).to(device) #A*A used it as Eq6,Feature Influence after 2 iteration
aax_feature = torch.mm(adj_matrix2,features) #A*A*X
aax_feature = np.array(aax_feature.to(device)) 
...
features_GCN = torch.FloatTensor(features_GCN).to(device)
...
similarity_feature = (similarity_feature - np.min(similarity_feature))/dis_range #Normilized Similarity
#chooose node
...
while True:
    """
    Iterate B (number of budget) and selecting best node
...
print(f'Max Validation Accuracy: {max_val} and Associated Test Accuracy: {max_test}')
#-------------
